                     ...
                     <Link key={product?._id} to={"/product/"+product?._id} className='w-full min-w-[280px] md:min-w-[300px] max-w-[280px] md:max-w-[300px] bg-white rounded-sm shadow ' onClick={scrollTop}>
                         ...
                         <div className='p-4 grid gap-3'>
                             ...
                             <div className='flex items-center gap-3'>
                                 {/* ✅ FIX: Shows sellingPrice if it exists, otherwise falls back to price */}
                                 <p className='text-red-600 font-bold'>
                                     { displayKSh(product?.sellingPrice > 0 ? product?.sellingPrice : product?.price) }
                                 </p>
                                 
                                 {/* ✅ Only show the line-through price if there is actually a discount */}
                                 {product?.sellingPrice > 0 && product?.sellingPrice < product?.price && (
                                     <p className='text-slate-500 line-through text-sm'>
                                         { displayKSh(product?.price) }
                                     </p>
                                 )}
                             </div>
